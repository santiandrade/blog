...
| `cardImage` | `{ src, alt: { es, en } }` | **Obligatoria en posts nuevos**: imagen editorial local de la card; aparece entre el link "Leer post"/"Read post" y las tags |
...
2. **Añade el objeto de metadata** en `js/data/posts-meta.js`, dentro del array `window.SITE_POSTS_META`. Insértalo en la posición correcta según fecha (el array va del más reciente al más antiguo, y `number` es correlativo, p. ej. `"02"` para el segundo post). Rellena todos los campos de la tabla de la sección anterior. **Todo post nuevo debe incluir `cardImage` con una imagen editorial local y `alt` ES/EN**; la home renderiza siempre título → extracto → link "Leer post"/"Read post" → imagen → tags (el link "Leer post" lo genera automáticamente `renderPostList()` en `js/main.js`, no hace falta añadirlo a la metadata). Si el post usa un tag que no existe todavía, añádelo primero a `js/data/tags.js`.
